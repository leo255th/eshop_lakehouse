...
TRUNCATE TABLE ads_eshop_100m.ads_trade_overview_day;
INSERT INTO ads_eshop_100m.ads_trade_overview_day
SELECT
  dt,
  ...
  NOW()                                                                   AS etl_time
FROM dws_eshop_100m.dws_trade_user_day
GROUP BY dt;

TRUNCATE TABLE ads_eshop_100m.ads_order_funnel_day;
INSERT INTO ads_eshop_100m.ads_order_funnel_day
SELECT
  dt,
  SUM(order_cnt)                                                          AS order_cnt,
...
TRUNCATE TABLE ads_eshop_100m.ads_category_rank_day;
INSERT INTO ads_eshop_100m.ads_category_rank_day
SELECT
  dt,
  category,
  rank_no,
...
FROM (
  ...
  FROM dws_eshop_100m.dws_trade_category_day
) t;

TRUNCATE TABLE ads_eshop_100m.ads_user_rfm;
INSERT INTO ads_eshop_100m.ads_user_rfm
SELECT
  user_id,
  recency_days,
  frequency,
...
FROM (
  ...
  FROM dws_eshop_100m.dws_user_lifetime
) t;

TRUNCATE TABLE ads_eshop_100m.ads_gmv_caliber_compare;
INSERT INTO ads_eshop_100m.ads_gmv_caliber_compare
SELECT
  p.dt,
  p.gmv_snapshot,
  p.gmv_current_dim,
...
FROM (
  ...
  FROM dws_eshop_100m.dws_trade_product_day
  GROUP BY dt
) p
LEFT JOIN (
  SELECT dt,
...
TRUNCATE TABLE ads_eshop_100m.ads_price_divergence_by_category;
INSERT INTO ads_eshop_100m.ads_price_divergence_by_category
SELECT
  category,
  SUM(item_rows)                                                          AS total_rows,
  SUM(diverged_rows)                                                      AS diverged_rows,
