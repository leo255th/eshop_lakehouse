...
TRUNCATE TABLE dws_eshop_1m.dws_trade_user_day;
INSERT INTO dws_eshop_1m.dws_trade_user_day
SELECT
  ...
  NOW()                                                         AS etl_time
FROM paimon_eshop_1m.dwd.fact_order_acc
GROUP BY 1, 2;

TRUNCATE TABLE dws_eshop_1m.dws_trade_product_day;
INSERT INTO dws_eshop_1m.dws_trade_product_day
SELECT
...
LEFT JOIN paimon_eshop_1m.dwd.dim_product p
...
TRUNCATE TABLE dws_eshop_1m.dws_trade_category_day;
INSERT INTO dws_eshop_1m.dws_trade_category_day
SELECT
  STR_TO_DATE(i.order_date, '%Y%m%d')                           AS dt,
  p.category,
  COUNT(DISTINCT i.order_id)                                    AS order_cnt,
  ...
  NOW()                                                         AS etl_time
FROM paimon_eshop_1m.dwd.fact_order_item i
JOIN paimon_eshop_1m.dwd.dim_product p ON p.product_id = i.product_id
GROUP BY 1, 2;

TRUNCATE TABLE dws_eshop_1m.dws_trade_province_day;
INSERT INTO dws_eshop_1m.dws_trade_province_day
SELECT
  ...
  NOW()                                                         AS etl_time
FROM paimon_eshop_1m.dwd.fact_order_acc o
JOIN paimon_eshop_1m.dwd.dim_user u ON u.user_id = o.user_id
GROUP BY 1, 2;

TRUNCATE TABLE dws_eshop_1m.dws_user_lifetime;
INSERT INTO dws_eshop_1m.dws_user_lifetime
SELECT
  user_id,
...
TRUNCATE TABLE dws_eshop_1m.dws_product_stock_day;
INSERT INTO dws_eshop_1m.dws_product_stock_day
SELECT
  s.dt,
  s.product_id,
  s.day_open_stock,
  ...
  s.change_cnt,
  NOW()                                                         AS etl_time
FROM paimon_eshop_1m.dwd.fact_product_stock_per_day s;
...
-- INSERT INTO dws_eshop_1m.dws_trade_user_day
-- SELECT STR_TO_DATE(order_date,'%Y%m%d'), user_id, ... FROM paimon_eshop_1m.dwd.fact_order_acc
