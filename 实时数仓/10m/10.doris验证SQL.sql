...
SELECT 'Paimon fact_order_item' AS layer, COUNT(*) AS rows_cnt FROM paimon_eshop_10m.dwd.fact_order_item
UNION ALL SELECT 'Paimon fact_order_acc',  COUNT(*) FROM paimon_eshop_10m.dwd.fact_order_acc
UNION ALL SELECT 'Paimon dim_user',        COUNT(*) FROM paimon_eshop_10m.dwd.dim_user
UNION ALL SELECT 'Paimon dim_product',     COUNT(*) FROM paimon_eshop_10m.dwd.dim_product
UNION ALL SELECT 'Paimon dim_price_scd2',  COUNT(*) FROM paimon_eshop_10m.dwd.dim_product_price_scd2;

SELECT 'Doris fact_order_item' AS layer, COUNT(*) AS rows_cnt FROM dwd_eshop_10m.fact_order_item
UNION ALL SELECT 'Doris fact_order_acc',  COUNT(*) FROM dwd_eshop_10m.fact_order_acc
UNION ALL SELECT 'Doris dim_user',        COUNT(*) FROM dwd_eshop_10m.dim_user
UNION ALL SELECT 'Doris dim_product',     COUNT(*) FROM dwd_eshop_10m.dim_product
UNION ALL SELECT 'Doris dim_price_scd2',  COUNT(*) FROM dwd_eshop_10m.dim_product_price_scd2;
...
SELECT
  ...
  (SELECT CAST(SUM(total_amount) AS DECIMAL(18,2))
     FROM paimon_eshop_10m.dwd.fact_order_acc)                     AS dwd_value,
  (SELECT CAST(SUM(order_amount) AS DECIMAL(18,2))
     FROM dws_eshop_10m.dws_trade_user_day)                        AS dws_value,
  (SELECT CAST(SUM(order_amount) AS DECIMAL(18,2)) FROM ads_eshop_10m.ads_trade_overview_day) AS ads_value
UNION ALL
SELECT
  '口径A 快照价',
  (SELECT CAST(SUM(quantity * unit_price) AS DECIMAL(18,2)) FROM paimon_eshop_10m.dwd.fact_order_item),
  (SELECT CAST(SUM(sale_amount_snapshot) AS DECIMAL(18,2)) FROM dws_eshop_10m.dws_trade_product_day),
  (SELECT CAST(SUM(gmv_snapshot) AS DECIMAL(18,2)) FROM ads_eshop_10m.ads_gmv_caliber_compare)
UNION ALL
SELECT
  '订单数',
  (SELECT CAST(COUNT(*) AS DECIMAL(18,2)) FROM paimon_eshop_10m.dwd.fact_order_acc),
  (SELECT CAST(SUM(order_cnt) AS DECIMAL(18,2)) FROM dws_eshop_10m.dws_trade_user_day),
  (SELECT CAST(SUM(order_cnt) AS DECIMAL(18,2)) FROM ads_eshop_10m.ads_trade_overview_day);
...
FROM ads_eshop_10m.ads_gmv_caliber_compare;
...
FROM ads_eshop_10m.ads_price_divergence_by_category
...
FROM ads_eshop_10m.ads_order_funnel_day;

SELECT COUNT(*) AS 超差天数 FROM (
  SELECT dt, SUM(gmv_share) AS s FROM ads_eshop_10m.ads_category_rank_day GROUP BY dt
) t WHERE ABS(s - 100) > 0.01;

SELECT COUNT(*) AS 排名异常天数 FROM (
  SELECT dt, COUNT(*) AS c, MAX(rank_no) AS mx, MIN(rank_no) AS mn
  FROM ads_eshop_10m.ads_category_rank_day GROUP BY dt
...
FROM ads_eshop_10m.ads_user_rfm;
...
FROM ads_eshop_10m.ads_user_rfm GROUP BY rfm_segment ORDER BY 用户数 DESC;

SELECT COUNT(*) AS aov不自洽天数 FROM ads_eshop_10m.ads_trade_overview_day
WHERE ABS(aov - pay_amount / NULLIF(pay_cnt, 0)) > 0.01;

SELECT
  (SELECT COUNT(*) FROM paimon_eshop_10m.dwd.fact_order_item)                 AS 明细原行数,
  (SELECT SUM(item_rows) FROM dws_eshop_10m.dws_trade_product_day)            AS DWS行数,
  (SELECT SUM(item_rows) FROM dws_eshop_10m.dws_trade_product_day)
    - (SELECT COUNT(*) FROM paimon_eshop_10m.dwd.fact_order_item)             AS 差异_应为0;

SELECT COUNT(*) AS 商品维重复行 FROM (
  SELECT product_id FROM paimon_eshop_10m.dwd.dim_product GROUP BY product_id HAVING COUNT(*) > 1
) t;

SELECT COUNT(*) AS 区间重叠数 FROM (
  SELECT s1.product_id
  FROM paimon_eshop_10m.dwd.dim_product_price_scd2 s1
  JOIN paimon_eshop_10m.dwd.dim_product_price_scd2 s2
...
SELECT
  '明细行数' AS metric,
  (SELECT COUNT(*) FROM paimon_eshop_10m.dwd.fact_order_item) AS A_直查Paimon,
  (SELECT COUNT(*) FROM dwd_eshop_10m.fact_order_item)        AS B_Doris内表,
  (SELECT COUNT(*) FROM paimon_eshop_10m.dwd.fact_order_item)
    - (SELECT COUNT(*) FROM dwd_eshop_10m.fact_order_item)     AS 差异_应为0
UNION ALL
SELECT '订单数',
  (SELECT COUNT(*) FROM paimon_eshop_10m.dwd.fact_order_acc),
  (SELECT COUNT(*) FROM dwd_eshop_10m.fact_order_acc),
  (SELECT COUNT(*) FROM paimon_eshop_10m.dwd.fact_order_acc)
    - (SELECT COUNT(*) FROM dwd_eshop_10m.fact_order_acc);
...
-- CREATE MATERIALIZED VIEW IF NOT EXISTS dwd_eshop_10m.mv_category_day AS
-- SELECT i.dt,
--        p.category,
...
--   SELECT * FROM dwd_eshop_10m.mv_category_day ORDER BY dt, category LIMIT 10;
--   SHOW MATERIALIZED VIEWS FROM dwd_eshop_10m;

SELECT
  (SELECT CAST(SUM(order_amount) AS DECIMAL(18,2)) FROM dws_eshop_10m.dws_trade_user_day) AS 下单GMV_重跑后,
  (SELECT CAST(SUM(gmv_snapshot) AS DECIMAL(18,2)) FROM ads_eshop_10m.ads_gmv_caliber_compare) AS 快照GMV_重跑后,
  (SELECT CAST(SUM(order_cnt) AS DECIMAL(18,2)) FROM ads_eshop_10m.ads_trade_overview_day) AS 订单数_重跑后;
